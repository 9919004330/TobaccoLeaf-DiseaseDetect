...
function forResult(imgData){
    ...
    document.getElementById('result-img').onload=function(){
        // console.log(document.getElementById('result-img').getBoundingClientRect().height)
        predictClass(document.getElementById('result-img'))
            .then(data => {
                document.querySelector('.cont-disease').textContent=data
                // console.log(data,window.innerHeight,document.getElementById('result-img').getBoundingClientRect().height);
            })
    }
}
